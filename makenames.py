...
def makename(male, female):
    if male and female:
        gender = random.choice(dict.keys(genders))
    elif male:
        gender = 'M'
    elif female:
        gender = 'F'
    ...
    return { 'first':  random.choice(genders[gender])
           , 'middle': random.choice(genders[gender])
           , 'last':   random.choice(genders[gender])
           }

def makenames(count, male, female):
    return [makename(male, female) for i in range(count)]

def ASUUsernameFormatter(person):
    chance = random.randint(0,99)

    suffix = ('4' if chance < 2
        else '3' if chance < 5
        else '2' if chance < 10
        else '1' if chance < 25
        else '')

    return (person['last'] + person['first'][0] + person['middle'][0] +
            suffix).lower()
...
for person in makestudents(args.number, args.male, args.female,
    ...
    print "%(id)d,%(username)s,%(email)s,%(first)s,%(middle)s,%(last)s" % person
